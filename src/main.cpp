...
GLUSboolean update(GLUSfloat time)
{
  try 
  {
	...
	setUniform(clothProgramId, "render_ind", g_clothShaderId);
	float3 light = float3(0, 10, 10);
    if (g_clothShaderId == 2)
    {
	  g_cloth.pMesh->Draw(GL_TRIANGLES);
    }
    else if (g_clothShaderId == 1)
    {
      ...
      g_cloth.pMesh->Draw(GL_LINES);  // draw connections
		...
		setUniform(clothProgramId, "light_pos", light);
 		GLint unf = glGetUniformLocation(clothProgramId, "floor_texture");
		glActiveTexture(GL_TEXTURE1);
		glUniform1i(unf, 1);
		glBindTexture(GL_TEXTURE_2D, g_cloth.texture);
		g_cloth.pMesh->Draw(GL_TRIANGLES);
	}
	...
	setUniform(g_program.program, "light_pos", light);
	
	GLint unf = glGetUniformLocation(g_program.program, "floor_texture");
	glActiveTexture(GL_TEXTURE1);
	glUniform1i(unf, 1);
...
int main(int argc, char* argv[])
{
  ...
  glusKeyFunc(keyboard);
	...
	if (!glusCreateWindow("Компьютерная графика, задание 4. Полыковский, 317г.", 1024, 768, GLUS_FALSE))
	{
  ...
  glGetError(); // flush error state variable, caused by glew errors
	...
	floorTexture = CreateGLTextureFromFile("../data/texture1.bmp");
	glusRun();
